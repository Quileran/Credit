...
<%
	...
	boolean isFormSubmitted=name!=null&&!name.isEmpty();
	boolean formValid=false;
	boolean requesSend = false;

	if (isFormSubmitted) {
		boolean nameValid = name!=null&&!name.isEmpty();
		boolean loginValid = login!=null&&!login.isEmpty();
		boolean passwordValid = password!=null&&!password.isEmpty();



		if (!nameValid) {
			errorMessage = "Поле имя пустое";
		}
		if (!loginValid) {
			errorMessage += "Поле логина пустое";
		}
		if (!passwordValid) {
			errorMessage += "Поле пароля пустое";
		}

		formValid= nameValid && loginValid && passwordValid;
		System.out.println("formValid = " +formValid);

		if (formValid) {
			boolean userExists=Helper.getInstance(). checkIsUserExists(login);
			...
			if (userExists) {
				...
				SendRequest sendRequest = new SendRequest(name, login, password);
				Helper.addRequest(sendRequest);
				requesSend=true;
			}
...
<head>
	...
	<title>LR Bank - Регистрация</title>
				...
				<div class="card shadow-lg">
					<div class="card-body p-5">
						...
						<%
							...
							if ( !requesSend ) {
						%>
						<center><h1 class="fs-4 card-title fw-bold mb-4">LR Bank Registration</h1></center>
						<form method="POST" class="needs-validation" novalidate="" autocomplete="off">
							...
							<div class="mb-3">
								...
								<input id="login" class="form-control" name="login" value="" required>
								<div class="invalid-feedback">
									Введите логин!
								</div>
							</div>
						...
						<div class="card-footer py-3 border-0">
							<div class="text-center">
								<a href="loginNewPage.jsp" class="text-dark">Login</a>
							</div>
						</div>
